...
def car():
    ...
    frame_count = 0
    global check_run_left, check_run_right, on_run_left, on_run_right
    global check_forward, check_right, check_left, check_backward, \
        check_stop, front_Distance, behind_Distance, left_Distance, right_Distance
    while True:
        if len(image) > 2:

            frame = cv2.resize(image[2], (int(width), int(height)))
            ...
            for result in results:
                ...
                if between and max_index_between > 330:
                    ...
                    if max_index_between > 440:
                        ...
                        if (((max_index_left < max_index_right and check_run_left) or check_run_right is False)
                                and on_run_left):
                            colorx = (0, 0, 255)
                            if left_Distance > 20:
                                run_left()
            ...
            print('Thoi gian di chuyen', time.time() - input)
...
read_thread.start()
process_thread = threading.Thread(target=car)
